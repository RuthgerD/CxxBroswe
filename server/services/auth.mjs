...
const getProviders = () => ({
  github: {
    ...
    redirectUri:
      ...
      "http://dev.cxxbroswe.xyz:8080/auth/callback", // Your client app URL
    service: {
...
export const loginGithub = async (req, res, next) => {
  ...
  if (request.clientId === provider.clientId) {
    ...
    const oathTokenReponse = await requestify.request(
      provider.service.tokenRequestUrl,
      { method: "POST", body: oauthRequestBody, headers: oauthRequestHeaders }
    );
    ...
    const userDetailsRequestHeaders = {
      Authorization: tokenType + " " + userAccessToken,
    };

    // Use temporary token to request user details from Github
    const userDetailsRequest = await requestify.request(
      provider.service.userDetailsRequestUrl,
      { method: "GET", headers: userDetailsRequestHeaders }
    );
    ...
    console.log(
      "User " + username + " w/ ID: " + userId + " logged in using " + "github"
    );
    ...
    let user = await UserService.getByExternalId(
      ghUser.external_id,
      (ghUser.source = "github")
    );
    if (!user) {
      const settings = await new Settings().save();
      ghUser.settings = settings._id;
    ...
    if (userId && username) {
      ...
      res
        .status(200)
        .json({ access_token: generateToken(user), userId: user.id });
    } else {
...
export const authoriseRequest = async (req, res, next) => {
  ...
  if (authorization && authorization.includes("Bearer")) {
    payload = decodeToken(authorization.replace("Bearer ", ""));
    if (payload) {
      user = await UserService.get(payload.id);
      if (user) {
